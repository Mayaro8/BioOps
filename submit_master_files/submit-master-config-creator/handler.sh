...
# seq_type=indep
# run_num=430
# batch_date_id=2020220
# batch_id=batch290126
# samples_ids_str=6upv58
# mongo_cluster_label=common
# cluster_num=4
# mode=final_checker
# exclude_samples=from_task
# mode_prs=tube
# batchids_prs=s1,s2,s3
...
if [[ $stage == 1 || $stage == "1" ]]; then
    ...
    echo -e "batchids_prs\t$batchids_prs" >> variables_s1.tsv
    echo -e "mode_prs\t$mode_prs" >> variables_s1.tsv
    ...
    echo -e "delete_files\t$delete_files" >> variables_s3.tsv
    echo -e "bad_only\t$bad_only" >> variables_s3.tsv
    echo -e "copy_bad\t$copy_bad" >> variables_s3.tsv
    echo -e "jobs_checker\t$jobs_checker" >> variables_s3.tsv
...
elif [[ $cluster_num == 5 || $cluster_num == "5"  ]]; then
    cluster=pipeline-v3-common-2
elif [[ $cluster_num == 6 || $cluster_num == "6"  ]]; then
    cluster=pipeline-v3-common-3
elif [[ $cluster_num == 7 || $cluster_num == "7"  ]]; then
...
elif [[ $mongo_cluster_label == "prod"  ]]; then
    mongo_cluster=analysis-pipeline-test
elif [[ $mongo_cluster_label == 5 || $mongo_cluster_label == "5"  ]]; then
    mongo_cluster=pipeline-v3-common-2
elif [[ $mongo_cluster_label == 6 || $mongo_cluster_label == "6"  ]]; then
    mongo_cluster=pipeline-v3-common-3
elif [[ $mongo_cluster_label == 7 || $mongo_cluster_label == "7"  ]]; then
    mongo_cluster=pipeline-v3-common-4
else
    echo "add mongo cluster label common or dev or prod. common is Argo 158.160.187 and dev is xx.131.x.18, also 5 6 7 represent common 2 3 4"
    exit 1
fi 
